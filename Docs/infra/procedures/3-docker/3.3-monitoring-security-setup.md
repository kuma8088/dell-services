...
# 手動実行テスト
sudo logwatch --detail Med --range today --output stdout | head -50
```
...
- **アクション**: `sudo logwatch --detail Med --range today` を実行する
- **期待結果**: ログレポートが生成され、エラーなく表示される
- **失敗時**: `/var/log/logwatch/` ディレクトリの権限確認、logwatch再インストール
