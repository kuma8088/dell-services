...
# バックアップファイルの確認
ls -lh /mnt/backup/docker-backup/$(date +\%Y\%m\%d)/
cat /var/log/docker-backup/backup.log | tail -20
```

**期待される出力**:
```
/mnt/backup/docker-backup/20240115/:
-rw-r--r--. 1 root root  50M ... docker-volumes.tar.gz
-rw-r--r--. 1 root root 100M ... docker-images.tar.gz
-rw-r--r--. 1 root root  1K  ... docker-compose-files.tar.gz
-rw-r--r--. 1 root root  5K  ... swarm-configs.tar.gz

[2024-01-15 14:30:01] バックアップ開始
[2024-01-15 14:30:05] Dockerボリュームのバックアップ: 成功
[2024-01-15 14:32:10] Dockerイメージのバックアップ: 成功
[2024-01-15 14:32:15] docker-composeファイルのバックアップ: 成功
[2024-01-15 14:32:20] Swarm設定のバックアップ: 成功
[2024-01-15 14:32:25] 古いバックアップの削除: 成功
[2024-01-15 14:32:30] バックアップ完了
```
...
cd /mnt/backup/docker-backup/$BACKUP_DATE/
sudo tar -xzf docker-volumes.tar.gz -C /
...
################### Logwatch 7.x.x (MM/DD/YY) ####################
 ...
 Container Started: 5 containers
 Container Stopped: 3 containers
 Image Pulled: 2 images
...
```bash
# テスト通知を送信
/usr/local/bin/send-alert.sh "TEST" "統合テスト: Discord通知テスト"
...
**コマンド**:
```bash
sudo grep -E "PermitRootLogin|PasswordAuthentication|PubkeyAuthentication" /etc/ssh/sshd_config | grep -v "^#"
```
...
**コマンド**:
```bash
ls -l /root/swarm-join-tokens.txt
cat /root/swarm-join-tokens.txt
```

**期待される出力**:
```
-rw-------. 1 root root 456 ... /root/swarm-join-tokens.txt
...
# 3. 本番データ削除（障害シミュレーション）
docker volume rm production_data
docker volume ls | grep production_data  # 存在しないことを確認
...
# 5. データ復元確認
docker volume ls | grep production_data
...
# 1. SSH不正アクセステスト
for i in {1..5}; do
  ssh invalid_user@<workstation_ip>
  sleep 2
done
...
docker run --name mem-test -d --memory="100m" --memory-swap="100m" \
  alpine sh -c "while true; do dd if=/dev/zero of=/dev/null bs=1M count=200; sleep 1; done"
...
docker run --rm --memory="50m" --memory-swap="50m" \
  alpine sh -c "dd if=/dev/zero of=/tmp/test bs=1M count=100"
...
# 6. クリーンアップ
docker stop mem-test cpu-test
docker rm mem-test cpu-test
```
...
**診断**:
```bash
cat /var/log/docker-backup/backup.log
ls -l /mnt/backup
```

**考えられる原因と対策**:
- **外付けHDD未接続**: `mount | grep /mnt/backup` で確認、HDD接続を確認
- **ディスク容量不足**: `df -h /mnt/backup` で容量確認、古いバックアップ削除
- **パーミッションエラー**: `sudo chown -R root:root /mnt/backup`
